    ...
    scheduleCollection.snapshots().listen((snapshot) {
      if (snapshot.exists) {
        ...
        List<Map<String, dynamic>> fetchedSchedule =
            List<Map<String, dynamic>>.from(snapshot.data()?['schedule'] ?? []);
                      ...
                      Expanded(
                        child: Padding(
                          ...
                          child: Card(
                            ...
                            color: (scheduleItem['schedule_user'] ?? []).any(
                                    (friend) =>
                                        friend['id'] ==
                                        'User1') // Check if User1 is included
                                ? const Color(0xFFFFB2A5).withOpacity(
                                    0.2) // Highlight color if User1 is included
                                : Colors.white, // Default color
                            elevation: 5, // Shadow effect for elevation
                            shadowColor:
                                Colors.grey.withOpacity(0.5), // Light shadow
                            child: ListTile(
                              contentPadding: const EdgeInsets.all(
                                  16.0), // Add padding inside ListTile
                              title: Text(
                              ...
                              subtitle: Row(
                                children: (scheduleItem['schedule_user'] ?? [])
                                    .map<Widget>((friend) {
                                  ...
                                  return Container(
                                    ...
                                    decoration: BoxDecoration(
                                      shape: BoxShape.circle,
                                      color: Colors.grey
                                          .shade300, // Light grey background for avatar
                                    ),
                                    child: Center(
                                      child: Text(
                                        name.isNotEmpty
                              ...
                              trailing: Column(
                                ...
                                children: [
                                  Text(
                                    scheduleItem['scheduleCreator']['id'] ==
                                            'User1'
                                        ? '나'
                                        : scheduleItem['scheduleCreator']
                                            ['name'],
                                    style: const TextStyle(
                                      fontSize: 14,
      ...
      bottomNavigationBar: schedule.isNotEmpty
          ? Padding(
              padding: const EdgeInsets.only(
                  bottom: 16.0,
                  left: 40.0,
                  right: 40.0), // 아래쪽 여백만 조정하고 양옆 여백도 추가
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: isCompeleted
                      ? Colors.grey[600]
                      : const Color(0xFFFFB2A5), // 상태에 따라 색 변경
                  shape: RoundedRectangleBorder(
                ...
                child: Text(
                  ...
                  style: TextStyle(
                    ...
                    fontWeight: FontWeight.bold,
                    color:
                        isCompeleted ? Colors.white : const Color(0xFF646464),
                  ),
                ),
              ),
            )
